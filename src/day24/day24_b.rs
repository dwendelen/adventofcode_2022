...
fn main() {
    ...
    let trip_1 = do_run(&mut grid, 0, 0, 0, rows - 1, cols - 1);
    let trip_2 = do_run(&mut grid, rows - 1, cols - 1, trip_1, 0, 0);
    let trip_3 = do_run(&mut grid, 0, 0, trip_2, rows - 1, cols - 1);
...
fn do_run(grid: &mut Vec<Vec<Cell>>, start_row: usize, start_col: usize, start_t: usize, end_row: usize, end_col: usize) -> usize {
    ...
    for t in (start_t + 1)..(start_t + 1 + steps) {
        let slot = t % steps;
        if grid[start_row][start_col].time_table[slot] {
            grid[start_row][start_col].fastest_table[slot] = t;
            todo.push((start_row, start_col, t))
        }
    }
