...
fn main() {
    ...
    let reducer: i64 = monkeys.iter().map(|m|m.test).fold(1, |a, b|a*b as i64);
    ...
    for _ in 0..10000 {
        for i in 0..amount_monkey {
            loop {
                ...
                let op = my_monkey.op;
                let new_val: i32 = (op(item, other_val) % reducer) as i32;
    ...
    let res = inspections[0] as i64 * inspections[1] as i64;
    println!("{res}")
}
