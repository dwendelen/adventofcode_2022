...
fn main() {
    ...
    for _ in 0..10 {
        ...
        positions.clear();
        for elf in &mut elfs {
            elf.update_positions(&proposals);
            positions.insert(elf.pos);
        }
    ...
    let ((r_min, r_max), (c_min, c_max)) = elfs.iter()
        .map(|e|e.pos)
    ...
    let res = (r_max - r_min + 1) * (c_max - c_min + 1) - elfs.len() as i32;
    println!("{}", res)
    // 4071 is too high
}
...
impl Elf {
    ...
    fn update_positions(&mut self, proposals: &HashMap<(i32, i32), usize>) {
